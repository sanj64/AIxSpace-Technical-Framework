        ...
        try:
            ...
            class _Env(gym.Env):
                ...
                def __init__(self_, scores: np.ndarray, threshold: float) -> None:
                    ...
                    self_.observation_space = spaces.Box(low=0.0, high=1.0, shape=(1,), dtype=np.float32)
                ...
                def step(self_, action):  # type: ignore[override]
                    ...
                    reward = 1.0 if (is_anomaly and action >= 1) or (not is_anomaly and action == 0) else -1.0
                    self_.index += 1
                    done = self_.index >= len(self_.scores)
                    obs = np.array([self_.scores[min(self_.index, len(self_.scores) - 1)]], dtype=np.float32)
...
class DecisionEngine:
    ...
    def train_rl(self, anomaly_scores: np.ndarray, seed: int = 42) -> None:
        ...
        try:
            ...
            self._rl_model = PPO("MlpPolicy", aenv.env, verbose=0, seed=seed)
            self._rl_model.learn(total_timesteps=self.rl_timesteps)
            self.rl_model_path.parent.mkdir(parents=True, exist_ok=True)
            self._rl_model.save(str(self.rl_model_path))
    ...
    def load_rl(self) -> None:
        ...
        try:
            ...
            self._rl_model = PPO.load(str(self.rl_model_path))
    ...
    def decide_rl(self, score: float) -> Decision:
        ...
        if self._rl_model is None:
            ...
            return Decision(action=action_str, reason="RL fallback: score-threshold rule", timestamp=ts)

        obs = np.array([[float(score)]], dtype=np.float32)
        action, _ = self._rl_model.predict(obs, deterministic=True)
        action_str = _ACTION_MAP.get(int(action), "LOG")
        ts = pd.Timestamp.utcnow()
        return Decision(action=action_str, reason=f"RL policy action={int(action)}", timestamp=ts)
    ...
    def _rule_decision(self, risk: RiskResult, phase: MissionPhase) -> Decision:
        ...
        if level == "MEDIUM":
            ...
            if sub in ("Thermal", "EPS") and phase.name in ("Launch", "Deployment", "Commissioning"):
                return Decision(
